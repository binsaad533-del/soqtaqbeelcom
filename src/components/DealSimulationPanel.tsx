  ...
  if (!open) {
    return (
      ...
      >
        ...
        <span className="text-[11px] font-medium text-foreground/80">محاكاة الصفقة — ماذا لو؟</span>
    ...
    <div className="rounded-xl border border-primary/15 bg-primary/[0.02] p-3 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <AiStar size={14} />
          <span className="text-xs font-semibold">محاكاة الصفقة</span>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          <div className="text-[9px] text-muted-foreground flex items-center gap-1">
            السعر الأساسي: <SarSymbol size={7} /> <span className="font-medium text-foreground">{result.basePrice.toLocaleString("en-US")}</span>
          </div>
            ...
            {result.scenarios.map((s, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 text-muted-foreground">
                  ...
                  <span>عمولة: {s.commission.toLocaleString("en-US")}</span>
                </div>
                ...
                {s.name === result.bestScenario && (
                  <div className="flex items-center gap-1 mt-1 text-success text-[9px] font-medium">
                    <ArrowRight size={8} /> أفضل سيناريو
                  </div>
                )}
              </div>
            ))}
