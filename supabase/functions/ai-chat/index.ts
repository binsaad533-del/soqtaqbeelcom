...
المستخدم ما يحتاج يروح لأي صفحة ثانية — أنت تسوي له كل شيء من هنا.
...
7. **🚨 لا توجّه المستخدم لأي صفحة أخرى أبداً** — أنت تسوي كل شيء من هنا مباشرة

═══════════════════════════════════════════════
تدفق إنشاء الإعلان الكامل (من الدردشة):
═══════════════════════════════════════════════

عندما المستخدم يبي يبيع أو ينزل إعلان:

1. **أنشئ المسودة** → استدعِ create_listing بالمعلومات المتوفرة
2. **اسأل عن الناقص واحد واحد**: السعر، المدينة، الحي
3. **عدّل المسودة** → استدعِ edit_my_listing لإضافة كل معلومة جديدة
4. **حدد الموقع** → استدعِ set_listing_location — تقبل رابط خريطة أو اسم المدينة والحي فقط (مثلاً city_name="الطائف" district_name="حي الدهاس")
5. **عندما يكون جاهز** → استدعِ publish_my_listing مباشرة
6. **بعد النشر** → أرسل رابط الإعلان المنشور للمستخدم

⚠️ مهم جداً: إذا المستخدم أعطاك اسم المدينة والحي، استدعِ set_listing_location مباشرة بـ city_name و district_name — لا تطلب رابط خريطة!

⚠️ إذا نقص شيء للنشر (صور، موقع، سعر): أخبر المستخدم بالضبط ماذا ينقص واطلبه منه مباشرة.
⚠️ الصور: المستخدم يقدر يرفع الصور هنا في الدردشة وأنت تربطها بالإعلان.

أمثلة على التفاعل المثالي:
━━━━━━━━━━━━━━━━━━━━━━
المستخدم: "أبي أبيع مطعمي بالرياض"
→ استدعِ create_listing فوراً (نشاط: مطعم، مدينة: الرياض)
→ "أنشأت المسودة ✅ — كم السعر المطلوب؟"
→ بعد ما يعطيك السعر: edit_my_listing
→ "ممتاز! وين الموقع؟ أرسل رابط قوقل ماب أو اسم المدينة والحي"
→ بعد ما يعطيك الرابط أو اسم المدينة+الحي: set_listing_location
→ "تبي أنشره الحين؟"
→ "نعم" → publish_my_listing → "تم نشر إعلانك 🎉 [رابط]"
...
1. استخدم الأدوات تلقائياً وفوراً — لا تقل "ما أقدر" أبداً ولا تؤجل التنفيذ
2. 🚨🚨🚨 عند طلب إنشاء إعلان أو بيع: يجب أن تستدعي أداة create_listing عبر tool_call — ممنوع أن تكتب رداً نصياً بدون استدعاء الأداة فعلياً
3. 🚨 ممنوع اختلاق روابط أو معرّفات إعلانات — كل الروابط يجب أن تأتي حصرياً من نتيجة tool_call
4. 🚨🚨 لا توجّه المستخدم أبداً لصفحة ثانية (مثل /create-listing) — أنت تسوي كل شيء هنا
5. في عمليات الحذف أو الإلغاء فقط: أكّد مع المستخدم
6. باقي العمليات: نفّذ عبر tool_call ثم أخبر المستخدم بالنتيجة
7. استخدم Markdown (جداول، قوائم، **غامق**)
8. بعد كل عملية، اقترح الخطوة التالية المنطقية
9. إذا المستخدم رفع ملف: حلله فوراً واستخرج كل المعلومات
10. العمولة = 1% على البائع
11. عند كشف علامات احتيال: نبّه فوراً
12. عند طلب النشر: استدعِ publish_my_listing — إذا فشل أخبر المستخدم بالمتطلبات الناقصة واطلبها منه
13. عند إعطاء رابط خريطة أو اسم مدينة+حي: استدعِ set_listing_location فوراً (الأداة تدعم الاسم النصي بدون رابط)
14. ⚡ إذا المستخدم قال اسم المدينة والحي: لا تطلب رابط — نفّذ set_listing_location مباشرة مع city_name و district_name
...
- لا تكشف أسماء الأدوات الداخلية للمستخدم أبداً
- **لا توجّه المستخدم لصفحة إنشاء الإعلان أو أي صفحة أخرى** — كل شيء يتم من هنا
...
const TOOL_DEFS: Record<string, any> = {
  ...
  set_listing_location: def("set_listing_location", "تحديد موقع الإعلان الجغرافي من إحداثيات أو رابط خريطة أو اسم مدينة وحي",
    { listing_id: { type: "string" }, lat: { type: "number", description: "خط العرض" },
      ...
      city_name: { type: "string", description: "اسم المدينة بالعربي أو الإنجليزي — مثلاً: الطائف" },
      district_name: { type: "string", description: "اسم الحي بالعربي أو الإنجليزي — مثلاً: حي الدهاس" } },
    ["listing_id"]),
...
async function executeTool(name: string, args: any, userId: string, role: string, sb: any): Promise<any> {
  switch (name) {
    ...
    case "generate_listing_card": {
      const { data: listing } = await sb.from("listings")
        .select("id, title, business_activity, city, district, price, deal_type, photos, ai_rating, ai_trust_score, area_sqm, status")
        .eq("id", args.listing_id).single();
      if (!listing) return { error: "الإعلان غير موجود" };
...
function isCanonicalResourcePath(pathname: string) {
  return RESOURCE_PATH_PATTERNS.some((pattern) => pattern.test(pathname));
}

function getFallbackOfficialUrl(pathname: string, officialUrlsByPath: Map<string, string>) {
  const resourceType = pathname.split("/").filter(Boolean)[0];
  if (!resourceType) return null;

  const matches = Array.from(officialUrlsByPath.entries())
...
function normalizeAssistantUrls(text: string, officialUrlsByPath: Map<string, string>) {
  if (!text) return "";
  return text.replace(RESOURCE_URL_PATTERN, (url) => normalizeToolUrl(url, officialUrlsByPath));
}

function collectOfficialUrlsFromValue(value: any, officialUrlsByPath: Map<string, string>) {
  if (!value) return;
  ...
  if (typeof value === "string") {
    try {
      ...
      const pathname = parsed.pathname.replace(/\/+$/, "") || "/";
      if (isCanonicalResourcePath(pathname)) {
        officialUrlsByPath.set(pathname, `${BASE_URL}${pathname}${parsed.search}${parsed.hash}`);
      }
    } catch {
  ...
  if (Array.isArray(value)) {
    value.forEach((item) => collectOfficialUrlsFromValue(item, officialUrlsByPath));
    return;
  }

  if (typeof value === "object") {
    Object.values(value).forEach((item) => collectOfficialUrlsFromValue(item, officialUrlsByPath));
  }
}

function extractOfficialUrlsFromToolMessages(messages: any[]) {
  const officialUrlsByPath = new Map<string, string>();

  for (const msg of messages) {
    if (msg?.role !== "tool" || typeof msg.content !== "string") continue;
    try {
      const parsed = JSON.parse(msg.content);
      collectOfficialUrlsFromValue(parsed, officialUrlsByPath);
    } catch {
...
function sanitizeHistoryMessage(msg: any) {
  if (msg.role === "assistant") {
    const cleaned = normalizeAssistantUrls(cleanAssistantText(extractMessageText(msg.content)), new Map());
    return {
      role: msg.role,
...
serve(async (req) => {
  ...
  try {
    ...
    // Only detect create/publish for auth gate — everything else is handled by default "required"
    ...
    // ONLY greetings/small talk get "auto" — EVERYTHING else forces tool execution
    const isConversationalOnly = /^(مرحبا|هلا|هلا والله|السلام عليكم|وعليكم السلام|كيفك|كيف حالك|شكرا|مشكور|الله يعطيك العافية|تسلم|يعطيك العافية|hi|hello|hey|thanks|thank you|صباح الخير|مساء الخير|شلونك)[\s!?.]*$/i.test(lastUserText.trim());
