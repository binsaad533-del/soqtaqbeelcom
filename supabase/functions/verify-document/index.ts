...
const FIELD_RULES: Record<string, { label: string; description: string; markers: string; rejectExamples: string }> = {
  ...
  ownership_proof: {
    label: "إثبات ملكية",
    description:
      "أي مستند تجاري يُعتبر دليلاً معقولاً على الملكية أو الشراء أو التعاقد، مثل: فواتير تجارية، فواتير ضريبية، عروض أسعار رسمية (Quotation)، إيصالات شراء، أوامر شراء (Purchase Order)، عقود بيع، عقود إيجار تجاري/سكني، سجل تجاري، رخصة نشاط، ترخيص بلدية، شهادة ملكية عقار/سيارة/معدة، أو أي مستند يحوي اسم جهة تجارية + تاريخ + أرقام + تفاصيل معاملة.",
    markers:
      "1) وجود اسم جهة تجارية (شركة/مؤسسة/متجر/مكتب) أو شعار/header\n2) تاريخ المعاملة\n3) أرقام (مبلغ، رقم فاتورة، رقم سجل، رقم عقد)\n4) تفاصيل معاملة (وصف بضاعة/خدمة/أصل/عقار)\n5) قد يكون بالعربية أو الإنجليزية أو ثنائي اللغة\n6) لا يلزم وجود ختم أو توقيع رسمي — فاتورة واضحة من شركة كافية",
    rejectExamples:
      "صورة شخصية (selfie)، طعام/مشروبات/مشاهد يومية، سيارة شخصية بدون مستند مرفق، منظر طبيعي، حيوانات، لقطة شاشة لدردشة واتساب أو تطبيق غير تجاري، صورة فاضية/سوداء/معطوبة، مستند مقطوع لدرجة لا تظهر منه أي معلومة قابلة للقراءة",
  },
};
...
const SYSTEM_PROMPT = (rule: typeof FIELD_RULES[string], expectedType: string) => {
  ...
  const isOwnershipProof = expectedType === "ownership_proof";
...
أجب باستخدام الأداة المتوفرة فقط.`;
  ...
  if (isOwnershipProof) {
    return `${baseHeader}
قواعد خاصة بإثبات الملكية (نطاق قبول موسّع):

✅ القبول (is_valid: true) — أي مستند تجاري يُعتبر دليلاً معقولاً، يشمل على سبيل المثال لا الحصر:
   - عقود موثقة بختم وتوقيع
   - فواتير تجارية من شركة/مؤسسة (حتى بدون ختم)
   - عروض أسعار رسمية (Quotation) بـ header شركة
   - إيصالات شراء من متاجر معروفة أو غير معروفة
   - سجل تجاري / رخصة نشاط / ترخيص بلدية
   - عقد إيجار تجاري أو سكني
   - شهادة ملكية عقار / سيارة / معدة
   - فاتورة ضريبية (Tax Invoice)
   - Purchase Order / Invoice / Receipt
   - إيصالات مكتوبة بخط اليد إذا حوت اسم جهة + تاريخ + أرقام معاملة
   - أي مستند يحتوي: اسم جهة تجارية + تاريخ + أرقام + تفاصيل معاملة

❌ الرفض (is_valid: false) — فقط للحالات الواضحة التالية:
   - selfie أو صورة وجه شخصية
   - طعام / مشروبات / مشاهد يومية
   - سيارة شخصية بدون مستند مرفق
   - منظر طبيعي / حيوانات
   - لقطة شاشة لدردشة واتساب أو تطبيق غير تجاري
   - صورة فاضية / سوداء كلياً / معطوبة
   - مستند مقطوع لدرجة لا تظهر منه أي معلومة قابلة للقراءة

⚠️ قاعدة مرنة إلزامية:
- لا ترفض لمجرد غياب الختم الرسمي أو التوقيع. فاتورة تجارية واضحة مع اسم شركة وتاريخ وأرقام = دليل مقبول.
- لا ترفض لمجرد أن المستند بالإنجليزية — اقبل المستندات الدولية وثنائية اللغة.
- لا ترفض لمجرد أن المستند ليس "سند ملكية رسمي" — أي وثيقة تجارية تربط بين جهة وأصل/خدمة مقبولة.
- الرفض فقط عندما يكون المحتوى واضحاً أنه ليس وثيقة تجارية إطلاقاً (صورة شخصية، طعام، منظر، إلخ).

عند الرفض، اكتب rejection_reason واضحاً وموجهاً للمستخدم بالعربية، مثل:
"الصورة تبدو شخصية (selfie) — الرجاء رفع مستند تجاري (فاتورة/عقد/سجل تجاري)"
"الملف لقطة شاشة لدردشة شخصية — الرجاء رفع فاتورة أو عقد رسمي"
